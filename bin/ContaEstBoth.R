...
ParseRow<-function(x){
	#observed number of ref bases
	refct<-as.numeric(x[which(bases==x[7])+1])
	#observed number of non-ref bases
	nonrefct<-sum(as.numeric(x[which(bases!=x[7])+1]))
	#observed number of alt bases
	altct<-as.numeric(x[which(bases==x[8])+1])
	#observed number of non-alt bases
	nonaltct<-sum(as.numeric(x[which(bases!=x[8])+1]))
	#reference frq
	reffrq<-as.numeric(x[9])
	#alternative frq
	altfrq<-(1-as.numeric(x[9]))
	return(c(refct, nonrefct, altct, nonaltct, reffrq, altfrq))
}

cnstable<<-t(apply(sites, 1, ParseRow))

#cnstable has 6 columns
#1. number of ref bases
#2. number of non-ref bases
#3. number of alt bases
#4. number of non-alt bases
#5. ref freq
#6. alt freq
...
TwoConsmLL<-function(conta){
	...
	fis<-(conta*cnstable[,6]*(1-((4/3)* MMrate)))+MMrate
	#cnstable[,2] is the number of non-ref alleles
	probs<-(choose((cnstable[,1]+cnstable[,2]), cnstable[,2])*(fis^cnstable[,2])*((1-fis)^cnstable[,1]))/2
	#use frq of the ref allele (not alt)
	fisinv<-(conta*cnstable[,5]*(1-((4/3)* MMrate)))+MMrate
	#cnstable[,4] is the number of non-alt bases
	probsinv<-(choose((cnstable[,3]+cnstable[,4]), cnstable[,4])*(fisinv^cnstable[,4])*((1-fisinv)^cnstable[,3]))/2
...
if(est2>0.1){
	warning("WARNING: Two-cns estimate greater than 10%! We have observed that contaminant population misspecification leads to underestimates in this contamination rate. Consider trying different contaminant populations. ")
}

#print a warning
if(est2>0.2){
	warning("WARNING: Two-cns estimate greater than 20%! Note that we have observed a downward bias when estimating contamination for data with a contamination fraction greater than 25%.")
}
